...
export function SwaggerFilter({ swaggers, onFilteredSwaggersChange }: SwaggerFilterProps) {
  ...
  const [activeProject, setActiveProject] = useState<string | null>(() => {
    ...
    const savedProject = localStorage.getItem(SWAGGER_PROJECT_KEY);
  ...
  // Filter function
  const filterSwaggers = useCallback(() => {
    ...
    onFilteredSwaggersChange(filtered)
  }, [swaggers, searchTerm, activeProject, onFilteredSwaggersChange])
  ...
  const handleProjectSelect = (project: string | null) => {
    ...
    if (project === null) {
      localStorage.removeItem(SWAGGER_PROJECT_KEY);
    } else {
      localStorage.setItem(SWAGGER_PROJECT_KEY, project);
    }
  }
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            {activeProject ? activeProject.substring(0, 10) + (activeProject.length > 10 ? '...' : '') : "Project"}
          </Button>
        </DropdownMenuTrigger>
